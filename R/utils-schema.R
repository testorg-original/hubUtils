...
#'   [schema](https://github.com/testorg-rename/schemas)
#'   (e.g. `"v0.0.1"`).
...
get_schema_url <- function(config = c("tasks", "admin", "model"),
  ...
  schema_repo <- "testorg-rename/schemas"
...
get_schema_valid_versions <- function(branch = "main") {
  branches <- gh::gh(
    "GET /repos/testorg-rename/schemas/branches"
  ) %>%
    vapply("[[", "", "name")
  ...
  if (!branch %in% branches) {
    cli::cli_abort(c(
      "x" = "{.val {branch}} is not a valid branch in schema repository
                   {.url https://github.com/testorg-rename/schemas/branches}",
      "i" = "Current valid branches are: {.val {branches}}"
    ))
  }

  req <- gh::gh("GET /repos/testorg-rename/schemas/git/trees/{branch}",
    branch = branch
  )
  ...
  if (inherits(response, "try-error")) {
    cli::cli_abort(
      "Connection to schema repository failed. Please check your internet connection.
            ...
            {.url https://github.com/testorg-rename/schemas}"
    )
  }
  ...
  if (schema_version %in% valid_versions) {
    ...
    cli::cli_abort(
      "{.val {schema_version}} is not a valid schema version.
            ...
            For more details, visit {.url https://github.com/testorg-rename/schemas}"
    )
  }
}
